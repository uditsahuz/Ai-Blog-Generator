...
// ALL client-side/browser code MUST use only the anon key!
// ALL server-side (API) code needing elevated privileges should use the service key, never ever expose it to the browser!
...
// --- Client-side protection: never leak service key ---
if (typeof window !== 'undefined' && supabaseServiceKey) {
  throw new Error('SECURITY: SUPABASE_SERVICE_KEY should never run in the browser!')
}
...
// Create clients with fallback to prevent build-time crashes
// Pages/API routes will handle missing env vars gracefully
export const supabase = (supabaseUrl && trimmedAnonKey)
  ? createClient(
      supabaseUrl.trim(),
      trimmedAnonKey,
      {
